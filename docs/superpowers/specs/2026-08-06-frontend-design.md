...
**`ExtractionSummary`** — renders once `/query` returns, built from `extracted_filters` + `extracted_profile`. Two chip groups, since only one of the two source objects has a corresponding backend filter surface:
- **Filter chips** (languages, fee-free, subject, city, course_type — i.e. every non-null field of `extracted_filters`) are removable/editable. Editing one updates local `SearchFilters` state and calls `useFilteredSearch` (`POST /search` with the original `semantic_query` and the adjusted filters) rather than re-running `/query` — correcting a bad extraction is instant and doesn't re-invoke the LLM parser or eligibility reasoner.
- **Profile chips** (degree field, grade + scale, nationality, other notes — i.e. every non-null field of `extracted_profile`) are read-only/informational. `student_profile` isn't part of `/search`'s request shape at all — it only feeds `/query`'s eligibility reasoning — so there is no instant, client-side action an edit to one of these could take; correcting a misread profile detail means rephrasing the original query and resubmitting through `/query`, not editing a chip.

A "start over" action resets all state back to the empty `ChatQueryBox`. When `extracted_filters`/`extracted_profile` come back `null` (Layer 2 parse-failure degradation on the backend), this renders a plain notice instead of either chip group — see Error Handling.
